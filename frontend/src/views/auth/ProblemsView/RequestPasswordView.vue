...
<template>
  <div class="space-y-2 mb-8 text-center">
    <h1 class="text-4xl font-bold text-pastel-lilac">¿Olvidaste tu Contraseña? ¿Bloqueaste tu cuenta?</h1>
    <p class="text-lg text-light-mauve/90">Ingresa tu email y te enviaremos las instrucciones.</p>
  </div>
  ...
  <div class="bg-deep-plum/60 border border-muted-grape/50 rounded-2xl p-8 shadow-xl backdrop-blur-sm">
    <form @submit="onSubmit" class="space-y-6">
      <div>
        ...
        <Field name="email" id="email" type="email" :class="inputClass('email')" placeholder="tu@email.com"/>
        <ErrorMessage name="email" class="mt-1.5 text-xs text-red-400" />
      </div>

      <button type="submit" :disabled="isSubmitting" class="w-full flex justify-center items-center py-3 px-4 rounded-full shadow-lg text-base font-bold text-deep-plum bg-gradient-to-r from-light-mauve to-pastel-lilac transform transition-transform duration-300 hover:scale-105 disabled:opacity-70 disabled:cursor-not-allowed">
